...
async function getPokeDetails(url) {
    try {
        ...
        console.error(`Error fetching details for Pokémon: ${url}`, error);
    }
}
...
async function getSpeciesDescription(speciesUrl) {
    try {
        ...
        console.error(`Error fetching species description: ${speciesUrl}`, error);
    }
}

// Picks three Pokémon at random and logs their name/description
async function logThreePkmn() {
    try {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const description = await getSpeciesDescription(details.species.url);
            console.log(`${pokemonName}: ${description}`);
        }
    } catch (error) {
        console.error('Error in logThreePkmn function:', error);
    }
}
